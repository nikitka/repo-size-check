...
FROM $join
WHERE (l_shipmode = 'MAIL' OR l_shipmode = 'TRUCK') AND l_commitdate < l_receiptdate AND l_shipdate < l_commitdate AND CAST(l_receiptdate AS timestamp) >= $border AND CAST(l_receiptdate AS timestamp) < ($border + Interval("P365D"))
GROUP BY
    l_shipmode
ORDER BY
    l_shipmode;
