...
$nd = ($dict) -> {
    RETURN ListSort(DictItems($dict), ($z) -> {
        RETURN $z.0
    })
};
SELECT
...
SELECT
    $nd(SetIntersection(AsDict(AsTuple(1, "foo"), AsTuple(3, "bar")), AsDict(AsTuple(1, "baz"), AsTuple(2, "qwe")), ($_k, $a, $b) -> {
        RETURN AsTuple($a, $b)
    }));
SELECT
...
SELECT
    $nd(SetUnion(AsDict(AsTuple(1, "foo"), AsTuple(3, "bar")), AsDict(AsTuple(1, "baz"), AsTuple(2, "qwe")), ($_k, $a, $b) -> {
        RETURN AsTuple($a, $b)
    }));
SELECT
    $ns(SetSymmetricDifference(ToSet(AsList(1, 2, 3)), ToSet(AsList(3, 4))));
SELECT
    $nd(SetSymmetricDifference(AsDict(AsTuple(1, "foo"), AsTuple(3, "bar")), AsDict(AsTuple(1, "baz"), AsTuple(2, "qwe")), ($_k, $a, $b) -> {
        RETURN AsTuple($a, $b)
    }));
