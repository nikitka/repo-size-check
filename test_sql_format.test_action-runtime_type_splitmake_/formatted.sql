...
$formatCallable = ($x) -> {
    RETURN AsStruct($x.OptionalArgumentsCount AS OptionalArgumentsCount, $x.Payload AS Payload, FormatType($x.Result) AS Result, ListMap($x.Arguments, $formatArgument) AS Arguments)
};
SELECT
    ...
    ListMap(TupleTypeComponents(ParseTypeHandle("Tuple<Int32,String>")), ($x) -> {
        RETURN FormatType($x)
    }),
    FormatType(TupleTypeHandle(ListMap(AsList("Int32", "String"), ($x) -> {
        RETURN ParseTypeHandle($x)
    }))),
    ListMap(StructTypeComponents(ParseTypeHandle("Struct<foo:Int32,bar:String>")), ($x) -> {
        RETURN AsTuple($x.Name, FormatType($x.Type))
    }),
    FormatType(StructTypeHandle(ListMap(AsList(AsTuple("foo", "Int32"), AsTuple("bar", "String")), ($x) -> {
        RETURN AsStruct($x.0 AS Name, ParseTypeHandle($x.1) AS Type)
    }))),
    StaticMap(DictTypeComponents(ParseTypeHandle("Dict<String,Int32>")), ($x) -> {
        RETURN FormatType($x)
    }),
