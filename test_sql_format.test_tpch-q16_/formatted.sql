...
WHERE p.p_brand <> 'Brand#33' AND (NOT StartsWith(p.p_type, 'PROMO POLISHED')) AND (p.p_size = 20 OR p.p_size = 27 OR p.p_size = 11 OR p.p_size = 45 OR p.p_size = 40 OR p.p_size = 41 OR p.p_size = 34 OR p.p_size = 36)
GROUP BY
    p.p_brand,
    p.p_type,
    p.p_size
