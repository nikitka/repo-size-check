...
$join4 = (
    ...
    JOIN plato.nation
        AS n
    ON j.s_nationkey = n.n_nationkey AND j.c_nationkey = n.n_nationkey
);
...
FROM $join5
WHERE r_name = 'AFRICA' AND CAST(o_orderdate AS Timestamp) >= $border AND CAST(o_orderdate AS Timestamp) < ($border + Interval("P365D"))
GROUP BY
    n_name
ORDER BY
    revenue DESC;
